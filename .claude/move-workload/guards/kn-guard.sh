...
# KN_* opt-ins below. Dependencies: kubectl; for `apply -f -` ONLY: jq and python3 (strict JSON validation) — if either is missing, apply is refused.
#
...
#   KN_CONTROLLER_KINDS="…"    optional: ADDITIONAL kinds that may be READ and DELETED but never patched/applied/annotated/labelled/scaled. The built-in Flux + ESO kinds (D1) are
#                              always included and can never be removed: an empty or different value changes nothing about them
...
#     --as-user-extra/--token/--raw/--insecure-skip-tls-verify/--tls-server-name/--certificate-authority/--client-*/--proxy-url/--cache-dir/--kuberc/--log-file/--log-dir/--v/--vmodule/
#     --username/--password/
...
#   * `apply` unless it is `apply -f -`. The manifest must be STRICT JSON, validated by python3's json module before anything else (YAML is refused; so are 0400, +1, .5, nan, infinity and
#     1e999 — kubectl's decoder falls back to YAML 1.1 and reads 0400 as 256, while jq would rewrite it to 400, so the bytes checked would not mean what kubectl reads). jq then canonicalises it, EVERY document (a stream is allowed) must be an object whose
...
_kn_L() {
 ...
 kustomizations.kustomize.toolkit.fluxcd.io helmreleases.helm.toolkit.fluxcd.io ocirepositories.source.toolkit.fluxcd.io externalsecrets.external-secrets.io";;
    ...
    LONG_DENY) echo "--namespace --all-namespaces --context --cluster --user --kubeconfig --server --as --as-group --as-uid --as-user-extra --token --raw --insecure-skip-tls-verify --tls-server-name --certificate-authority --username --password --filename --kustomize --recursive --proxy-url --cache-dir --kuberc --log-file --log-dir --log-file-max-size --log-backtrace-at --log-flush-frequency --v --vmodule";;
...
_kn_kinds_ok() {
  ...
  local -a parts
  kinds=${KN_KINDS:-$(_kn_L DEFAULT_KINDS)}; ctl="$(_kn_L CONTROLLER_KINDS) ${KN_CONTROLLER_KINDS:-}"      # ADDITIVE: the knob can only add kinds, never remove a built-in one
...
# _kn_apply_canon <apiVersion/Kind list>: manifest on stdin -> canonical `jq -c` stream on stdout, or a refusal. STRICT JSON only (python3 validates first); EVERY document is
# checked; apiVersion and kind are compared as separate fields; no content is echoed.
_kn_apply_canon() {
  local IFS=$' \t\n' raw canon e rc
  if ! command -v jq >/dev/null 2>&1; then _kn_refuse "apply needs jq to parse the manifest; refusing"; return 1; fi
  if ! command -v python3 >/dev/null 2>&1; then _kn_refuse "apply needs python3 to validate the manifest as STRICT JSON (jq's parser is lenient); refusing"; return 1; fi
  ...
  raw=$(cat)
  # STRICT validation BEFORE canonicalising. jq -c accepts 0400, +1, .5, nan, infinity and REWRITES them, so the bytes kubectl would have read (its decoder falls back to
  # YAML 1.1: 0400 is 256) would differ from the bytes we send (400). Python's json module with constants/float overflow rejected is a strict RFC 8259 parser.
  printf '%s' "$raw" | python3 -c '
import sys, json, math
def bad(x): raise ValueError("non-JSON constant " + x)
def pf(t):
    v = float(t)
    if math.isinf(v) or math.isnan(v): raise ValueError("float out of range")
    return v
dec = json.JSONDecoder(parse_constant=bad, parse_float=pf, strict=True)
s = sys.stdin.buffer.read().decode("utf-8"); i = 0; n = 0
while True:
    while i < len(s) and s[i] in " \t\n\r": i += 1
    if i >= len(s): break
    _, i = dec.raw_decode(s, i); n += 1
sys.exit(0 if n >= 1 else 3)' >/dev/null 2>&1; rc=$?
  case "$rc" in 0) ;; 3) _kn_refuse "apply: no document in the manifest"; return 1;; *) _kn_refuse "apply: the manifest is not STRICT JSON (YAML, 0400, +1, .5, nan, infinity and the like are refused: kubectl would read them differently from any parser here)"; return 1;; esac
  canon=$(printf '%s\n' "$raw" | jq -c '.' 2>/dev/null) || { _kn_refuse "apply: the manifest is not valid JSON"; return 1; }
  [ -n "$canon" ] || { _kn_refuse "apply: no document in the manifest"; return 1; }
  printf '%s\n' "$canon" | jq -e -s --arg allowed "$1" '
      ($allowed | split(" ") | map(select(length > 0)) | map({av: .[:rindex("/")], k: .[(rindex("/") + 1):]})) as $ok
      | length >= 1
      and all(.[];
          type == "object"
          and ((.kind | type) == "string") and ((.apiVersion | type) == "string")
          and (. as $d | any($ok[]; .av == $d.apiVersion and .k == $d.kind))
